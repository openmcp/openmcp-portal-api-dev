	...
	for _, element := range clusterData["items"].([]interface{}) {
		...
		for _, element := range nodeItems {
			/*
				name,
				status,
				role(node-role.kubernates.io/master < true),
				systemversion(osImage,(containerRuntimeVertion)),
				pods(capacity)
			*/
		...
		// http://192.168.0.152:31635/api/v1/pods?clustername=openmcp
		...
		for _, element := range podItems {
			nodeName := element.(map[string]interface{})["spec"].(map[string]interface{})["nodeName"].(string)
			podsCount[nodeName]++
		}
	}
